/**
 * @file robot2.cpp
 * @brief robot mechanism for wood1 league
 */
...
private:
	...
	typedef enum module_st_t {
        module_st_samples_pre,    // Travel
		module_st_samples_get,    // Collect
...
string Robot::act(vector<Sample> samples)
{
	...
	*/
	ostringstream res;
	switch (this->status) {
    case module_st_samples_pre:
		this->status = module_st_samples_get;
		res << "CONNECT 2"; // TODO: review strategy
		break;
	case module_st_samples_get:
	...
	case module_st_start_pos:
		this->status = module_st_samples_pre;
		res << "GOTO " << module_st2txt(this->status);
		break;
	}
	return res.str();
...
void Robot::setSample(const vector<Sample> & samples)
{
	...
	for (auto s : samples) {
        //s.debug();
		if (s.getCarriedBy() == Sample::carried_by_me) {
            this->sample = s;
            break;
        }
	}
